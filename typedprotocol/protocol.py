...
import abc
import inspect
from .substitution import TypeVarSubstitutor, SubstitutedMethod
...
class TypedProtocolMeta(abc.ABCMeta):
            ...
            if callable(attr_value):
                ...
                for param_name, param in method_sig.parameters.items():
                    if param_name != 'self' and param.annotation is param.empty:
                        raise TypeError(f"Parameter '{param_name}' in method '{attr_value.__name__}' of TypedProtocol '{cls.__name__}' must have a type annotation.")
                ...
                raise TypeError(f"Attribute '{attr_name}' in TypedProtocol '{cls.__name__}' must have a type annotation.")

    
    def __new__(cls, name: str, bases: tuple[type, ...], namespace: dict[str, typing.Any], **kwargs: typing.Any) -> type:
        namespace["__slots__"] = ()
        ...
        for base in bases:
            ...
            if base is not TypedProtocol and not isinstance(base, TypedProtocolMeta):
                raise TypeError(f"TypedProtocol '{name}' cannot inherit from non-protocol type '{base.__name__}'. "
                                f"TypedProtocols can only inherit from other TypedProtocols.")
        new_class.validate_annotations()
        return new_class
    
    def __call__(cls, *args: typing.Any, **kwargs: typing.Any) -> typing.NoReturn:
        """Prevent instantiation of all TypedProtocol classes."""
        raise TypeError(f"Cannot instantiate protocol class {cls.__name__}. "
    ...
    def __subclasscheck__(cls, subclass: type) -> bool:
        ...
        type_var_mapping: dict[typing.TypeVar, typing.Any] = {}
            ...
            if base is not TypedProtocol:
                if hasattr(base, '__annotations__'):
                    for attr_name, attr_type in base.__annotations__.items():
                        if substitutions:
                            protocol_annotations[attr_name] = TypeVarSubstitutor.substitute(attr_type, substitutions)
                        else:
                    ...
                    if not attr_name.startswith('_'):
                        ...
                        if callable(attr_value) and hasattr(attr_value, '__annotations__'):
                            if substitutions:
                                ...
                                protocol_methods[attr_name] = SubstitutedMethod(attr_value, substituted_annotations)
                            else:
        ...
        for attr_name, expected_type in protocol_annotations.items():
            ...
            if not hasattr(subclass, '__annotations__') or attr_name not in subclass.__annotations__:
                return False
            
            actual_type = subclass.__annotations__[attr_name]
            if not TypeChecker.is_compatible_with_unification(actual_type, expected_type, type_var_mapping):
                return False
        ...
        for method_name, protocol_method in protocol_methods.items():
            ...
            if not MethodChecker.are_compatible_with_unification(subclass_method, protocol_method, type_var_mapping):
                return False
...
class TypedProtocol(typing.Generic[T], metaclass=TypedProtocolMeta):
